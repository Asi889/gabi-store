...
export default function ClothingPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const clothingProducts = products.filter((p: any) => 
        p.categories?.some((c: any) => c.slug.toLowerCase().includes('clothing')) ||
        p.name.toLowerCase().includes('shirt') || 
        p.name.toLowerCase().includes('dress') ||
        p.name.toLowerCase().includes('clothing')
      );
      
      setAllProducts(clothingProducts);
      setFilteredProducts(clothingProducts);
      
      // 1. Extract Categories (only from clothing products)
      const cats: any[] = [];
